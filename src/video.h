...
namespace video {
  ...
  struct config_t {
    ...
    int videoFormat;  // codec_e wire value; PyroWave requires its versioned extension.
    ...
    // Set only after explicit negotiation of the private PyroWave extension.
    std::uint32_t pyrowave_protocol_version = 0;
    std::uint32_t pyrowave_path_mtu = 1280;
    std::uint32_t pyrowave_frame_budget = 0;
    std::uint32_t pyrowave_wire_byte_budget = 0;
    std::uint32_t pyrowave_fec_percentage = 0;
    std::uint32_t pyrowave_encoder_target_bytes = 0;
...
#undef _CONVERT
    ...
    // This is the standard encoder table. PyroWave has its own backend/probe;
    // neither it nor an unknown value may be silently treated as H.264.
  ...
  // Standard encoder capabilities only. PyroWave never indexes this array.
  extern std::array<bool, standard_codec_count> last_encoder_probe_supported_yuv444_for_codec;
  ...
  struct advertised_encoder_capabilities_t {
    int hevc_mode = 0;
    int av1_mode = 0;
    std::array<bool, standard_codec_count> yuv444_for_codec {};
  };
